...
class BoardMessage(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        haveShelter = Shelter.objects.filter(manager=self.request.user)
        if haveShelter.exists():
            filters = {'shelter':haveShelter[0]}
        else:
            filters = {'sender':self.request.user}
        context.update({
            # 'chatRooms': ChatRoom.objects.filter(sender=self.request.user).order_by('-date_created')
...
class AnswerMessageAjax(LoginRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        message = Message.objects.filter(chatroom__slug=self.kwargs['slug'])
        ...
        return JsonResponse({
            ...
            'user_sender': message[0].sender.username.title(), 
...
class CreateMessage(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form): 
        ...
        newChat = ChatRoom.objects.filter(shelter= pet.shelter, sender= self.request.user, pet= pet)
